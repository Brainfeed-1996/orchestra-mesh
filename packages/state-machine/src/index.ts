import type { WorkflowEvent } from '@orchestra-mesh/event-store/interfaces.js';
...
export interface WorkflowStateProjectionOptions {
...
export function projectWorkflowState(
  ...
  options: WorkflowStateProjectionOptions = {}
): WorkflowState {
  if (events.length === 0) {
...
function applyEvent(state: WorkflowState, event: WorkflowEvent): WorkflowState {
  ...
  if (event.status) {
    newState.status = event.status as WorkflowStatus;
  }
  ...
  switch (event.type) {
    ...
    case 'task.started':
      ...
      newState.currentStep = event.payload?.stepName as string | undefined;
      break;

    case 'task.completed':
      addOrUpdateStep(newState, event.payload?.stepName as string, 'completed', event.timestamp);
      break;

    case 'task.failed':
      addOrUpdateStep(newState, event.payload?.stepName as string, 'failed', event.timestamp, event.payload?.error as string);
      newState.status = 'failed';
      newState.error = event.payload?.error as string | undefined;
      break;
    ...
    case 'approval.rejected':
      ...
      newState.error = event.payload?.reason as string | undefined;
      break;
    ...
    case 'retry.attempts':
      newState.retryCount = (event.payload?.count as number) || state.retryCount + 1;
      break;
  }
...
function computeMetrics(events: WorkflowEvent[], state: WorkflowState): WorkflowMetrics {
...
function countStatusTransitions(events: WorkflowEvent[]): number {
  let transitions = 0;
